...
import com.example.rohanjuneja_jaikunwarsidhu_comp304_lab4.Models.NurseRepository;
...
public class NurseViewModel extends AndroidViewModel {
    ...
    public boolean checkLogin(int nurseId, String password){
        for(int i=0;i<allNurses.getValue().size();i++){
            if(allNurses.getValue().get(i).getNurseId()==nurseId &&  allNurses.getValue().get(i).getPassword().equals(password)){ return true; }}
        return false;
